...
cols=['Sent1','Prep1','Env1','Mov1','CollOr1']
...
def setUserOrder():
	...
	for f in os.listdir(folder):
		...
		for col in df.columns:
			...
			clipped=resdf[resdf.flatind>=start] 
			all_tags+=sum(clipped[col])
...
#Plot the vertical line for the landfall
plt.axvline(x=188, hold=True,color='r',linewidth=2)#Octobert 29th, 8PM
...
for u in users:
	...
	for col in cols:
		res=grouped[col].agg(lambda x: x[x!='nan'].value_counts().index[0] \
		if len(x[x!='nan'])>0 else 0)
		resdf=pandas.DataFrame(res)
		resdf['flatind']=range(len(res))
		resdf=resdf[resdf[col]!=0] #drop times where nothing happens
